...
> Every neighborhood in Sweden receives a score from 0 to 100 that reflects its current conditions and likely trajectory over the coming years. The score combines official government data across multiple dimensions — economics, education, safety, and financial stability — into a single number designed to help you understand where an area stands and where it's heading.
>
...
```
  ...
  ────────────────────────
  │  SCB  │  Skolverket  │  BRÅ  │  Kronofogden  │
  ────────────────────────
          ↓
...
> Every input to our model comes from Swedish government agencies: SCB, Skolverket, BRÅ, Kronofogden, and Polisen. We don't use proprietary data, surveys of our own, or user-submitted reviews. This means our scores are reproducible, auditable, and grounded in the same statistics that policymakers rely on.
...
> All our inputs are aggregate statistics — averages, rates, and percentages across entire areas. We do not access, store, or process any data about individual people. This is a deliberate design choice, both for legal compliance and because aggregate patterns are what drive neighborhood-level trends.
...
- Every data source by name (SCB, Skolverket, BRÅ, Kronofogden, Polisen)
- Every category of measurement (income, employment, education, schools, safety, financial distress)
- That the score is a percentile-based relative ranking
- That factors are weighted by relevance to real estate outcomes
- That all data is public and aggregate-level
